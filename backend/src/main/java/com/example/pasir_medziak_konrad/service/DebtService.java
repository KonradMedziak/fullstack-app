...
import com.example.pasir_medziak_konrad.dto.DebtDTO;
import com.example.pasir_medziak_konrad.model.*;
...
public class DebtService {
    ...
    public Debt createDebt(DebtDTO debtDTO){
        ...
        User debtor = userRepository.findById(debtDTO.getDebtorId()).orElseThrow(()->new EntityNotFoundException("Nie znaleziono dłuznika o ID: "+debtDTO.getDebtorId()));
    ...
    public void deleteDebt(Long debtId, User currentUser) {
        Debt debt = debtRepository.findById(debtId).orElseThrow(()->new EntityNotFoundException("Dług o ID "+debtId+" nie istnieje"));
    ...
    public boolean markAsPaid(Long debtId, User user) {
        Debt debt = debtRepository.findById(debtId).orElseThrow(()->new EntityNotFoundException("Nie znaleziono grupy"));
    ...
    public boolean confirmPayment(Long debtId, User user) {
        Debt debt = debtRepository.findById(debtId).orElseThrow(()->new EntityNotFoundException("Nie znaleziono długu"));
